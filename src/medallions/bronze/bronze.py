...
def clean_tiingo_eod(df: ps.DataFrame) -> ps.DataFrame:
    """Takes a pyspark DataFrame created from the Tiingo EOD API call, reformats the date column and removes any duplciate rows.
...
def check_tiingo_eod_missing(df: ps.DataFrame) -> None:
    """Takes a pyspark dataframe created from a Tiingo EOD API call and raises an error if there are any NA or NULL values. 
